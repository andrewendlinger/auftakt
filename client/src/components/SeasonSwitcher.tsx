...
import type { ID, Season } from '../api/types';
...
export function SeasonSwitcher() {
  ...
  const create = async (payload: {
    ...
    const season = await api.createSeason(payload.label, {
...
function NewSeasonModal({
  ...
  seasons: Season[];
  onSubmit: (p: { label: string; copyFrom?: number; includeProjects: boolean; includeTasks: boolean }) => void | Promise<void>;
  onClose: () => void;
}) {
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <Label>Künstler übernehmen aus</Label>
        ...
        {copyFrom && (
          <div className="space-y-2 rounded-lg bg-neutral-50 p-3">
            <p className="text-xs text-neutral-500">
              Künstler (und ihre Kontakte) werden übernommen. Zusätzlich mitkopieren:
            </p>
