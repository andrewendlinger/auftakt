...
- for anything that goes wrong during startup: the `Auftakt-Diagnose-<Kennung>.txt` that same
  dialog writes to your desktop when you report a *Fehler*, and asks you to attach. It carries
  `app-log.jsonl` in two sections — *Startprotokoll*, every boot line there is, the only record of
  what the boot animation actually did, and *Laufzeitprotokoll*, the most recent of whatever the
  app caught going wrong while it ran — plus the machine's details
