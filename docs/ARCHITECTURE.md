...
only `ipcRenderer.on` (`electron/preload.ts` → `onBackupConfigChanged`). WP-54's diagnostics trio
(`get-diagnostics`, `reveal-diagnostics`, `save-diagnostics`) is `invoke`/`handle` like everything
else, so that remains true. The first two take **no argument**: main derives `boot-log.jsonl`'s path
from `app.getPath('userData')` and hands the renderer finished summary text plus a path it may only
