...
| `useAnchoredPopover()` (`lib/popover.ts`) | any new popover: anchor rect, flip-above, clamp, height cap, close on scroll/resize, Escape — but a layout reflow that moves the anchor with no scroll behind it **repositions** the menu rather than closing it (a `ResizeObserver` on the anchor and its scroll ancestors, #176). Escape is a **capture-phase window listener, not a React `onKeyDown`** — a popover opened by a click has no focus inside its menu. Its menu renders at **`POPOVER_LAYER` (`z-[55]`)**, above the toast — see „Other client conventions". |
