...
export function copySeasonData(targetId: number, sourceId: number, opts: SeasonCopyOptions): void {
  ...
  try {
    ...
    // `erledigt_am` is the one stamp column COPY_COLS carries (the others are regenerated by the
    // target's own local DEFAULT). A source season that has never been active never ran
    // migrateStampsToLocal, so its stamps are still UTC — and createSeason marks the target
    // `stamps_localtime = '1'`, where nothing will ever convert them. Convert on the way in,
    // through the source's own SQLite so the semantics match the migration exactly.
    if (getSetting(source, STAMPS_LOCAL_KEY) !== '1') {
      const toLocal = source.prepare("SELECT COALESCE(datetime(?, 'localtime'), ?) AS v");
      for (const t of tasks) {
        if (typeof t.erledigt_am !== 'string') continue;
        t.erledigt_am = (toLocal.get(t.erledigt_am, t.erledigt_am) as { v: string }).v;
      }
    }
